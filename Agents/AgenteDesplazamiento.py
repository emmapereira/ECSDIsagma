    ...
    if '|' not in mess:
        ...
        if messtype not in ['BUSQTRANS']:
            ...
            if messtype == 'BUSQTRANS':
                log.info('Recibo petición de búsqueda de desplazamiento')
                ...
                if len(param) == 6:
                    ...
                    transps = encontrarTrans(checkindate, checkoutdate, adults, code, maxflightprice)
                    log.info(transps)
                else:
...
def info():
    ...
    return render_template('busquedas_transportes.html', busq=busquedas)
